...
def train(model, optimizer, train_loader, val_loader, full_neighbor_sampler, num_neighbors, num_epochs, save_path, dataset_name, early_stop_patience=10, best_ap=0.0):
    # 训练函数：训练模型并实现早停机制
    # best_ap = 0  
    ...
    for epoch in range(num_epochs):
        ...
        if current_mrr > best_ap:
            ...
            print(f'  -> New best MRR: {best_ap:.6f}, model saved!')
        else:
            patience_counter += 1
            print(f'  -> No improvement for {patience_counter} epoch(s), best MRR: {best_ap:.6f}')

        # 保存最新模型
        jt.save(model.state_dict(), f'{save_path}/{dataset_name}_CRAFT.pkl')
        if patience_counter >= early_stop_patience:
            print(f'\nEarly stopping triggered after {epoch+1} epochs!')
            print(f'Best validation MRR: {best_ap:.6f}')
            break
...
parser.add_argument('--resume', action='store_true', help='是否从上次保存的最佳模型继续训练')
...
initial_best_ap = 0.0
if args.resume:
    best_model_path = f'{save_path}/{args.dataset}_CRAFT.pkl'
    if os.path.exists(best_model_path):
        print(f'\n[Resume] 找到已保存的模型，正在加载: {best_model_path}')
        model.load_state_dict(jt.load(best_model_path))
        
        # 为了防止覆盖，先用加载的模型在验证集上跑一次，获取初始分数
        print('[Resume] 正在评估旧模型的初始 MRR 分数...')
        initial_val_res = test_val_mrr(model, val_loader, full_neighbor_sampler, num_neighbors)
        initial_best_ap = initial_val_res['MRR']
        print(f'[Resume] 加载完毕！当前模型的起点 MRR 为: {initial_best_ap:.6f}')
    else:
        print(f'\n[Resume] 未找到保存的模型 ({best_model_path})，将从头开始训练。')
# ======================================

# 训练模型
print(f'\nTraining for {args.epochs} epoch(s) with early stopping (patience={args.early_stop})...')
# 注意这里把 initial_best_ap 传给 train 函数了
best_ap = train(model, optimizer, train_loader, val_loader, full_neighbor_sampler, num_neighbors, args.epochs, save_path, args.dataset, args.early_stop, best_ap=initial_best_ap)
# 加载最佳模型进行预测
