    ...
    def execute(self, data):
        ...
        for it in range(batch_size):
            ...
            if not self.batch_update:
                ...
                for j in [u_it, v_it]:
                    for at in range(self.n_assoc_types):
                        self.node_degree_global[at][j] = jittor.sum(self.A[j, :, at]>0).item()
        ...
        if self.batch_update:
            batch_embeddings_u = jittor.stack(batch_embeddings_u, dim=0)
            batch_embeddings_v = jittor.stack(batch_embeddings_v, dim=0)
        ...
        for cnt, (v,u) in enumerate([(node_u,  node_v), (node_v, node_u)]):
            for at in range(self.n_assoc_types):
                ...
                if num_u_nb > 0:
                    ...
                    q_ui = jittor.exp(self.S[u, u_nb, at])
