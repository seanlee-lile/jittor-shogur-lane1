...
class GCN(nn.Module):
    ...
    def execute(self, graph, feat):
        for i in range(self.num_layers):
            feat = self.act_fn(self.convs[i](feat, graph.csc, graph.csr))
        return feat
...
class Grace(nn.Module):
    r"""GRACE model from the `"Deep Graph Contrastive Representation Learning"
    <https://arxiv.org/abs/2006.04131>`_ paper
    ...
    def sim(self, z1, z2):
        ...
        s = jt.matmul(z1, jt.transpose(z2))
        return s

    def get_loss(self, z1, z2):
        # calculate SimCLR loss
        f = lambda x: jt.exp(x / self.temp)

        refl_sim = f(self.sim(z1, z1))  # intra-view pairs
        between_sim = f(self.sim(z1, z2))  # inter-view pairs

        # between_sim.diag(): positive pairs
        x1 = refl_sim.sum(dim=1) + between_sim.sum(dim=1) - jt.diag(refl_sim)
        loss = -jt.log(jt.diag(between_sim) / x1)
    ...
    def execute(self, graph1, graph2, feat1, feat2):
        # encoding
