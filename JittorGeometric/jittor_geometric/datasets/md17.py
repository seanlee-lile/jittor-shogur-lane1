...
class MD17(InMemoryDataset):
    ...
    .. note::
        ...
        constructed via the :obj:`jittor_geometric.transforms.RadiusGraph`
    ...
    **STATS:**
    ...
    def __init__(
        ...
        self.data, self.slices = jt.load(self.processed_paths[idx])
    ...
    def process(self) -> None:
        ...
        for raw_path, processed_path in it:
            ...
            if self.revised:
                z = jt.array(raw_data['nuclear_charges'].astype(np.int64))
                pos = jt.array(raw_data['coords'].astype(np.float32))
                energy = jt.array(raw_data['energies'].astype(np.float32))
                force = jt.array(raw_data['forces'].astype(np.float32))
            else:
                z = jt.array(raw_data['z'].astype(np.int64))
                pos = jt.array(raw_data['R'].astype(np.float32))
                energy = jt.array(raw_data['E'].astype(np.float32))
                force = jt.array(raw_data['F'].astype(np.float32))
            ...
            jt.save(self.collate(data_list), processed_path)
